#!/usr/bin/env bash
# Push .env + production defaults to Vercel project env (REST API).
# Usage: ./scripts/push-vercel-env.sh [production|preview]
set -euo pipefail
...
secret_keys = set(vercel_cfg.get("deploySecrets", []))
...
print(f"Done. Synced {synced} env vars to Vercel ({target}).")
PY
